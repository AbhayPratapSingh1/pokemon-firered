...
import { buildStairsVisual, buildParapets, getGroundHeight } from "./GaryStairs.js";
...
/**
 * Builds Gary's House interior (mirror of Ash's House).
 * Uses Teleporter for door entry/exit.
 *
 * @param {THREE.Scene} scene
 * @param {THREE.Vector3} exteriorDoorWorldPos
 * @returns {object} controller
 */
export function setupGaryHouse(scene, exteriorDoorWorldPos) {
  ...
  // --- Stairs (mirrored, visual) -------------------------------------------
  buildStairsVisual(houseGroup);

  // --- Furniture from config (mirrored positions) --------------------------
  ...
  // Force world matrices so collision boxes land at HOUSE_ORIGIN.
  ...
  const interactables = [
    getTVInteractable(HOUSE_ORIGIN.x + HALF_W - 1.05, HOUSE_ORIGIN.z - HALF_D + 0.75),
